...
- 面向清北班的高考 3500 词：暮色主题一次展示一个词，沉静主题在右上角的一张卡片内上下展示两个词条；按词条展示中文释义、词族、词形和常用搭配
- 换词间隔支持常用预设和 5–600 秒自定义，可直接点击数字面板输入
- 底部工具栏：音乐播放器、今日作业和壁纸设置
- 本机音乐、LRC 歌词和网易云搜索
- Lively 暂停壁纸时，同步暂停背景视频、音乐和单词轮换

词库的核查范围、修正内容与参考资料见 [2026-09 词库校订记录](data/words/REVIEW-2026-09.md)。
...
> 列表右侧的刻度条支持点击定位；搜歌既可键盘输入后按回车，也可点击「粘贴搜索」。需要在桌面输入文字时，请在 Lively 中开启键盘输入；只使用点击与粘贴时无需开启。使「其他应用获得焦点时」暂停壁纸的选项位于 Lively「性能」设置。
...
- **音乐**：打开播放器，切换本机歌单或搜索网易云；点击列表右侧刻度条可跳转到对应位置。
- **作业缩放**：使用窗口顶部的缩放控件，最多放大至 400%；放大后可直接拖动图片查看，点击「适应」恢复完整显示。
- **作业**：点击选择、更换或清空今日作业图片，不使用拖入操作。
- **设置**：在外观、学习、音乐、关于四个分页中修改日期、时钟、背景播放、界面缩放和网易云登录。
...
npm run lively:reload   # 同步网页到当前 Lively 壁纸并刷新，无需打包
...
抽取权重为：所有词基础权重 `1.0`；进阶词额外 `+1.0`，挑战词额外 `+2.0`；有词族、重要搭配或易混关系时再分别增加 `+0.3`、`+0.2`、`+0.2`。因此每个词都可出现，同时更适合清北班的 285 个重点词会适度提高频率。最近 18 个学习组仍会被短期避开；实际抽取概率随可用学习组和词库校订变化。
