...
$usbRoot = Join-Path $distRoot "XWalnut-usb"
...
Copy-Item -LiteralPath (Join-Path $distRoot "XWalnut-lively.zip") -Destination $usbRoot
Copy-Item -LiteralPath (Join-Path $distRoot "XWalnut-music-setup.exe") -Destination $usbRoot

$instructions = @"
XWalnut USB package

1. Install Lively Wallpaper.
2. Import XWalnut-lively.zip into Lively.
3. Double-click XWalnut-music-setup.exe for NetEase search and playback.
...
Write-Host "[XWalnut] USB package -> $usbRoot"
