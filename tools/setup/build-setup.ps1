...
function Ensure-InnoLanguageFiles {
    ...
    foreach ($suffix in @("ChineseSimplified", "ChineseTraditional")) {
        ...
        try {
            ...
            Write-Host "[XWalnut] Warning: could not fetch $suffix.isl for Inno Setup."
        }
    }
}
...
function Bootstrap-InnoPortable {
    ...
    Write-Host "[XWalnut] Downloading Inno Setup portable..."
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
...
function Get-LivelySetupFile {
    if ($env:LIVELY_SETUP_EXE -and (Test-Path -LiteralPath $env:LIVELY_SETUP_EXE)) {
        Write-Host "[XWalnut] Using LIVELY_SETUP_EXE: $env:LIVELY_SETUP_EXE"
    ...
    if (Test-Path -LiteralPath $downloadsCandidate) {
        Write-Host "[XWalnut] Using Downloads cache: $downloadsCandidate"
        return $downloadsCandidate
    }
    $cached = Join-Path $livelyCacheDir "lively_setup_x86_full_v2210.exe"
    if (Test-Path -LiteralPath $cached) {
        Write-Host "[XWalnut] Using build cache: $cached"
        return $cached
    }
    New-Item -ItemType Directory -Path $livelyCacheDir -Force | Out-Null
    Write-Host "[XWalnut] Downloading Lively Wallpaper $LivelyVersion installer (~208 MB) from GitHub..."
    [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
...
Write-Host "[XWalnut] Step 1/5: building Lively wallpaper package..."
...
Write-Host "[XWalnut] Step 2/5: building bridge-payload.zip..."
...
Copy-Item -LiteralPath (Join-Path $distRoot "XWalnut-lively.zip") -Destination $payloadRoot -Force
Copy-Item -LiteralPath (Join-Path $distRoot "bridge-payload.zip") -Destination $payloadRoot -Force
foreach ($script in @("lively-install.ps1", "download-lively.ps1", "uninstall-xwalnut.ps1")) {
    Copy-Item -LiteralPath (Join-Path $scriptDir $script) -Destination $scriptsRoot -Force
}

Write-Host "[XWalnut] Step 3/5: embedding Lively installer..."
...
Write-Host "[XWalnut] Step 4/5: locating Inno Setup..."
$iscc = Find-ISCC
if (-not $iscc) {
    Write-Host "[XWalnut] Inno Setup not found; installing portable copy..."
...
Write-Host "[XWalnut] ISCC: $iscc"

$version = Get-PackageVersion
Write-Host "[XWalnut] Step 5/5: compiling installer (version $version)..."
...
Write-Host "[XWalnut] Unified installer -> $outputPath ($sizeMb MB)"
